...
public class FlightBase {
    ...
    List<String> getArrivalCitiesFrom(String city) {
        ...
        if (city.isEmpty()) {
            ...
            flights.entrySet().stream()
                    .filter(e -> (e.getValue()))
                    .filter(e -> e.getKey().getDepartureCity().contains(city))
    ...
    List<String> getDepartureCitiesTo(String city) {
        ...
        if (city.isEmpty()) {
            ...
            flights.entrySet().stream()
                    .filter(e -> (e.getValue()))
                    .filter(e -> e.getKey().getArrivalCity().contains(city))
                    .map(Map.Entry::getKey)
    ...
    boolean isPossible(String departureCity, String stopoverCity, String arrivalCity) {
        ...
        if (departureCity.isEmpty() || arrivalCity.isEmpty()) {
            ...
            if (stopoverCity.isEmpty()) {
                flights.entrySet().stream()
                        .filter(e -> (e.getValue()))
                        .filter(e -> e.getKey().getArrivalCity().equals(arrivalCity))
                        .filter(e -> e.getKey().getDepartureCity().equals(departureCity))
                        .map(Map.Entry::getKey)
                        .map(Flight::getArrivalCity)
                        .forEach(citiesWAS::add);
                boolean b = citiesWAS.size() == 0;
                ...
                flights.entrySet().stream()
                        .filter(a -> (a.getValue()))
                        .filter(a -> a.getKey().getArrivalCity().equals(arrivalCity))
                        .map(Map.Entry::getKey)
                        .map(Flight::getDepartureCity)
                        .forEach(citiesB::add);
